...
pub async fn list_channels(client: &Client) -> Result<Vec<ChannelInfo>, Error> {
    let data = client.exec_command_with_response("channellist", 5_000).await?;
    Ok(data
        .into_iter()
        .map(|item| {
...
pub async fn list_clients(client: &Client) -> Result<Vec<ClientInfo>, Error> {
    let data = client.exec_command_with_response("clientlist -uid -away -voice -groups", 5_000).await?;
    Ok(data
        .into_iter()
        .map(|item| {
